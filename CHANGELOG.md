...
- Round 34 runtime/parser hardening (2026-09-19): fixed PoleKnig Easy-mode annotations to prefer real visible synopsis text over SEO meta text; stopped rounded metadata durations from truncating one-file-per-chapter audio; added bounded segmented-worker joins for Windows cancellation; normalized retained source extensions; made Knigavuhe restricted pages metadata-consistent; hardened JS-call parsing, fallback resume cleanup, boolean settings, atomic cover writes, Audioknigi author-prefix cleanup, provider error statuses, Easy-mode stale-download state, player visibility, retry-all filtering, shutdown-safe result rendering, keyboard context menus, and dynamic localization. Current suite: 483 passed; all release gates green.
