...
- Round 54 edge-case hardening (2026-09-26): prevent support-bundle log tails from exposing private path fragments when a byte window starts mid-line; reject fractional/non-finite/boolean track selections while accepting scalar integer indices; audit Qt icon+text positional overloads and constructor f-strings; and fail closed on malformed acceptance schema types instead of crashing. Added focused regression coverage for each case; packaged Windows runtime and manual NVDA/JAWS acceptance remain separate gates.

- Round 53 audit-tool and diagnostics hardening (2026-09-26): restore nonempty short Windows CRLF log tails; preserve chapter zero in missing-source diagnostics; resolve exception allowlists from the requested root and detect broad tuple/ellipsis handlers; audit keyword UI text and the Qt entry point; report missing localization sources explicitly; preserve intentional test configuration fixtures while excluding private runtime state and compiled binaries from source archives; interpret all/* disk-space selections as the entire book and reject invalid indices; and show existing acceptance evidence when its candidate manifest is rejected without granting PASS or rewriting the report. Restore Round 49/48 changelog order. Validation: 648 tests passed, including 19 new regression cases; Windows packaged runtime and screen-reader acceptance remain separate checks.

- Round 52 audit/Windows hardening (2026-09-24): harden support-bundle privacy for embedded Windows drive/UNC filenames containing spaces or multiple dots while preserving trailing diagnostic text; precompile path-redaction regexes and accept pathlib.Path inputs; preserve the legacy auto-chunk speed value when the canonical key is present but null; normalize invalid/zero segment counts back to auto; normalize a None Qt exit code to success; skip malformed internal track indices instead of crashing book processing; and reject boolean selected-track indices instead of silently converting True/False to 1/0. Reviewed the remaining report findings as existing contracts, theoretical/non-runtime notes, or already-correct code. Windows-source review found no Windows-11-only API dependency; frozen EXE acceptance remains a Windows-side gate. Split verification: 623 functional/source tests + 6 static-quality wrappers = 629/629 passed; historical regression 255/128; full parity 61/61; all static gates green.

- Round 51 full Easy/Advanced search-state synchronization (2026-09-24): make search-result visibility a shared state instead of two independent view flags; mirror the selected result row between Easy and Advanced tables; automatically open the Advanced Search tab when switching modes while pending search results are active; preserve the same results when switching back to Easy; synchronize the Easy universal field, Advanced Book field, and Advanced Search query field; and end the shared result-selection stage when a result is chosen for analysis. Full suite: 622 passed; Round 50/51 sync regressions 10/10; historical regression audit 255/128; full parity 61/61; all static quality gates green.

- Round 50 Easy/Advanced synchronization and pre-analysis layout/AudioKnigi synopsis repair (2026-09-24): keep the universal Easy input and Advanced Book input synchronized bidirectionally; make the Advanced Book empty state the sole vertically elastic region before analysis so labels/inputs no longer drift apart on tall windows while the analysed chapter table keeps its existing stretch behavior; and anchor audioknigi.com.ua annotations to the visible “краткое содержание” section, removing the site SEO bridge, app/player/catalog metadata and stopping before listen/reviews/comments sections. Full suite: 617 passed; Round 50 + related regressions 21/21; release gates green.

- Round 49 audit hardening (2026-09-24): close the support-bundle string-path privacy gap for Windows paths with spaces; hash modern nested queue book URLs and accept versioned queue wrappers; normalize direct `AppSettings(...)` construction without disturbing legacy-load migration semantics; align downloader Knigavuhe fallback narrator matching with the analysis service; make resume guards finite-safe for NaN durations; clarify Smart Format copy logging; complete provider typing; and align the Easy card minimum with the declared 900 px window minimum. Full suite: 612 passed; Round 49 regressions 8/8; historical regression audit 255/128; full parity 61/61; all static quality gates green.

- Round 48 Advanced-layout repair (2026-09-23): stabilize the Book tab after analysis by removing post-population whole-table auto-sizing, bounding the synopsis in a scrollable text area, keeping long narration names from widening the window, grouping book metadata into a compact card, and hiding result/progress-only controls when inactive. Full suite: 604 passed; focused UI contracts 27/27; historical regression audit 255/128; full parity 61/61; all static quality gates green.

- Round 47 fifth-audit hardening (2026-09-23): preserve complete multi-line diagnostic logs while still masking Windows/UNC paths when home discovery fails; make `AppSettings` a fully consistent `MutableMapping` by materializing defaults; recognize legacy-only profiles as already onboarded; normalize string track indices during disk-space estimation; distinguish ambiguous AudioKnigi title-derived authors from trusted metadata so character/series names do not pollute authors or truncate titles; reuse the plain-text narrator fallback in direct AudioKnigi analysis; remove consecutive direct-analysis coauthor prefixes; accept `selected_indices="all"`/`"*"` in unfinished manifests; accept integer-like decimal strings in `safe_int` without truncating fractions; and make the localized delete action compatible with `QAction.triggered(bool)`. Full suite: 598 passed; historical regression audit 255/128; full parity 61/61; all static quality gates green.

- Round 46 fourth-audit hardening (2026-09-22): stop nested Knigavuhe metadata names leaking into authors/readers; normalize queue `parts` aliases and persisted track-selection booleans; remove consecutive verified AudioKnigi coauthor prefixes; make Easy search independent of hidden Advanced source filters; preserve shared settings-object identity; accept quoted Windows `.url` targets; stabilize missing-media dialog parenting and active-queue drag behavior; guard null native media-event pointers; and resynchronize full-MP3 selections after playlist refresh. Full suite: 586 passed; historical regression audit 255/128; full parity 61/61; all static quality gates green.

- Round 45 third-audit hardening (2026-09-21): preserve first-run completion for legacy profiles; recursively sanitize nested support-bundle settings and quoted secrets; resolve root-relative AudioKnigi links; keep player-position I/O ordered without holding the state lock; reject malformed shared-source chapter boundaries and zero-length PoleKnig marker inference; harden Windows media-key teardown, Audiobookshelf exit timing and zero-size Range geometry; release playing files before re-download; defer unfinished-library scans until path editing completes; and debounce keyboard seeking for Windows multimedia stability. Full suite: 574 passed; historical regression audit 255/128; full parity 61/61; all static quality gates green.

- Round 44 persistence/privacy/cancellation hardening (2026-09-20): sanitized diagnostic log tails before support-bundle packaging; made atomic temp names per-call unique; closed FFmpeg pipes deterministically; hardened Audioknigi initial matching, relative links and legacy encodings; serialized player-position persistence; bounded inline queue covers; made system-sound shutdown independent of the sentinel; disconnected accessibility trace callbacks before close; preserved recent asynchronous seek targets; and avoided resetting requests pools when Playwright session material is unchanged. Partial direct `AppSettings` mappings now provide documented defaults without breaking their existing Mapping/migration semantics. Full suite: 560 passed; historical regression audit 256/127; full parity 61/61; all static quality gates green.

- Round 43 parser/query/runtime hardening (2026-09-20): fixed Knigavuhe initial matching and `/0.mp3?query` filtering, protected PoleKnig backtick literals, hardened boolean/default settings and direct `AppSettings()` defaults, accepted whitespace-tolerant `Content-Range`, rejected empty audio source URLs before download planning, aligned dash-based identity parsing, improved Audioknigi initial/full-name author merging and narrator fallback boundaries, and removed two dead-code remnants. Reviewed the remaining audit findings as intentional contracts, already-safe behavior, or non-runtime maintenance notes. Full suite: 547 passed; historical regression audit 256/127; full parity 61/61; all static quality gates green.

- Round 42 light-theme accessibility polish (2026-09-20): use a blue external focus ring in Light/System-light while retaining amber in Dark/System-dark, strengthen light card borders, soften light table selection, and add a pale row-hover state.

- Round 42 Windows system-theme follow-up (2026-09-20): fixed mixed dark/light rendering after switching to System theme by keeping the native Qt style while applying a coherent palette that matches the OS-selected light/dark scheme; live system-scheme changes use the same rule.

- Round 42 Windows system-theme/accessibility follow-up (2026-09-20): stabilized the System theme on Windows by using Qt color-scheme detection (with palette fallback) and mapping light/dark appearance to the app production tokens instead of mixing QSS with native Base/Mid/Button colors; removed the redundant mode selector from Settings; fixed sound-preview semantics with a media-only packaged `search_complete` path that can never fall back to `MessageBeep`, while the system-sound button explicitly uses native Windows `MessageBeep`; widened the centered Settings card; and replaced focus-border mutations with an external `QFocusFrame` overlay hardened against dialog/menu teardown. Windows source accessibility self-test contract was also updated to stop requiring the removed legacy `ui_mode` widget ID; it now relies on `ui_mode_easy`, `ui_mode_advanced` and `ui_mode_stack`. Full suite: 536 passed; historical regression audit passed (256 historical tests, 127 known shape incompatibilities); all static quality gates green.
...
- Round 38 multi-author/accessibility follow-up (2026-09-19): fixed `audioknigi.com.ua` multi-author labels such as `Бурносов Юрий, Бурносова Татьяна - Тоннельная крыса` so only the book title remains in the Title column and all detected authors are preserved; direct-link analysis uses the same normalization. Added an explicit Easy-mode Tab chain, Tab-through behavior for read-only text views, audit coverage for Easy narration/annotation controls, and a high-visibility keyboard-focus ring for buttons, edits, combos, tables, lists, checkboxes, sliders and tabs in system/light/dark themes. Effective suite: 507 passed; all release gates green.

- Round 37 search relevance/coauthor follow-up (2026-09-19): Knigavuhe now applies a final title/author/narrator relevance filter after metadata hydration, preventing description/genre-only hits such as `Крауч Энд`, `Дворец грез`, and `Кому жить на Руси хорошо` from appearing for the query `Крыса` while preserving close inflections such as `Крыса` → `Крысы в стенах`. Audioknigi metadata now preserves card-detected coauthors instead of overwriting them with a single structured author, and strips only independently corroborated author prefixes from titles so `Бурносова Татьяна - Тоннельная крыса` renders as `Тоннельная крыса` with both authors while ambiguous dashed titles remain intact.

- Round 36 search/metadata/UI follow-up (2026-09-19): substantially widened the Easy-mode card while keeping the seven-column result table adaptive; hardened audioknigi.com.ua title cleanup for SEO suffixes/site URLs and author-prefixed titles without separators; revalidated every hydrated Audioknigi result against the user query so unrelated recommendations do not leak into searches; added source-specific synopsis extraction/SEO rejection for Audioknigi and Knigavuhe while preserving the PoleKnig annotation contract. Both Easy and Advanced modes continue to render the same canonical `Book.description`.

- Round 35 repository housekeeping (2026-09-19): moved incremental audit notes out of the repository root into `audits/4.12/rounds/`, added a round index and `.gitignore`, removed generated historical-regression worktrees from source packaging, introduced a clean source-ZIP packager, and added architecture checks for root/layout hygiene. No runtime behavior change.

- Round 34 runtime/parser hardening (2026-09-19): fixed PoleKnig Easy-mode annotations to prefer the real visible synopsis over SEO meta text; stopped rounded metadata durations from truncating one-file-per-chapter audio; waited for segmented workers to release Windows file handles after cancellation; normalized retained full-source extensions by container/codec; made Knigavuhe restricted pages return metadata-only books consistently; hardened BookController call parsing, fallback resume cleanup, boolean settings, atomic cover writes, Audioknigi author-prefix cleanup, provider error statuses, queue DnD coordinates, Easy-mode stale-download state, player visibility, retry-all filtering, shutdown-safe search rendering, keyboard context-menu selection, and dynamic localization. Current suite: 483 passed; all release gates green.
...
- Round 31 Easy-mode follow-up (2026-09-18): manual Backspace/Delete clearing of the Easy input now performs the same safe reset as “Скачать следующую книгу” when no search/analysis/download is active. The selected-book card also exposes the analyzed book annotation in a separate accessible scrollable description block, hidden when a provider supplies no description.

- Round 30 UI follow-up (2026-09-18): Easy mode now exposes narrator/recording selection for search results with multiple narration variants instead of silently analyzing the first URL. Multi-variant rows show an accessible `Озвучка` combo, require an explicit choice before analysis, preserve the full variant list into the analyzed Book, and keep the existing one-click flow for single-variant results.

- Round 29 UI follow-up (2026-09-18): made the Easy-mode seven-column search table fit within the existing centered card without horizontal scrolling. Title/author/narrator columns stretch responsively, compact service columns use content width, and long visual text elides while full cell values remain available through the model/accessibility layer.

- Round 28 hotfix (2026-09-18): replaced queued connections to Python mixin callables with a dedicated main-thread `WorkerUiRelay(QObject)`. Worker signals now target real QObject slots and only then forward to search/analysis/download/Audiobookshelf UI handlers, fixing the 5% stall where background search completed but progress/result callbacks never reached the GUI.

- Round 27 hotfix (2026-09-18): fixed the confirmed Qt cross-thread GUI violation revealed by Windows logs. Search, analysis, download and Audiobookshelf worker callbacks that mutate UI are now explicitly delivered with `Qt.ConnectionType.QueuedConnection`, including thread-finished cleanup callbacks. Added regression coverage preventing implicit worker-to-window AutoConnection wiring.

- Round 26 hotfix (2026-09-18): replaced the Windows Easy-mode native application-modal progress dialog with a modeless progress window plus explicit main-UI blocking. Completion now hides/deletes the progress window without `accept()`/native modal unwind, re-enables the UI, and logs exact begin/end teardown boundaries.
...
- Aligned Knigavuhe script-fallback narration variants with the normal parser, made PoleKnig alternative-author matching tolerate initials/surname forms conservatively, and let explicitly directory-shaped missing support-bundle destinations receive timestamped ZIPs. Added 8 focused Round 21 regressions; full suite: 378 passed. Historical regression remains 268/115 known source-shape incompatibilities; all release gates green.
