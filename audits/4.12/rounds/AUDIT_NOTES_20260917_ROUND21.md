...
- `compileall`: **PASS**.
- Windows CI follow-up: installs `pytest` before historical/full pytest gates, installs FFmpeg for archived loudnorm contracts, recognizes Python 3.14 synthetic `__conditional_annotations__`, and makes diagnostics privacy/newline assertions platform-neutral without weakening the runtime redaction policy.
