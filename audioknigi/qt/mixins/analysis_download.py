...
class AnalysisDownloadUiMixin:
    ...
    def start_analysis(self):
        ...
        worker.progress.connect(self._worker_ui_relay.analysis_progress, Qt.ConnectionType.QueuedConnection)
        worker.finished.connect(self._worker_ui_relay.analysis_finished, Qt.ConnectionType.QueuedConnection)
        worker.finished.connect(thread.quit)
        worker.finished.connect(worker.deleteLater)
        thread.finished.connect(self._worker_ui_relay.analysis_thread_finished, Qt.ConnectionType.QueuedConnection)
    ...
    def _launch_download(self, request, settings: dict, *, from_queue: bool = False, mode: str = "selected"):
        ...
        worker.moveToThread(thread)
        thread.started.connect(worker.run)
        worker.status.connect(self._worker_ui_relay.download_status, Qt.ConnectionType.QueuedConnection)
        worker.log.connect(self._worker_ui_relay.download_log, Qt.ConnectionType.QueuedConnection)
        worker.stage.connect(self._worker_ui_relay.download_stage, Qt.ConnectionType.QueuedConnection)
        worker.progress.connect(self._worker_ui_relay.download_progress, Qt.ConnectionType.QueuedConnection)
        worker.transfer.connect(self._worker_ui_relay.download_transfer, Qt.ConnectionType.QueuedConnection)
        worker.missing_media.connect(self._worker_ui_relay.download_missing_media, Qt.ConnectionType.QueuedConnection)
        worker.history_changed.connect(self._worker_ui_relay.download_history_changed, Qt.ConnectionType.QueuedConnection)
        worker.request_changed.connect(self._worker_ui_relay.download_request_changed, Qt.ConnectionType.QueuedConnection)
        worker.finished.connect(self._worker_ui_relay.download_finished, Qt.ConnectionType.QueuedConnection)
        worker.finished.connect(thread.quit)
        worker.finished.connect(worker.deleteLater)
        thread.finished.connect(self._worker_ui_relay.download_thread_finished, Qt.ConnectionType.QueuedConnection)
        thread.finished.connect(thread.deleteLater)
