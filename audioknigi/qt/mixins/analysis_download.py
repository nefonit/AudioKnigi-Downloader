...
class AnalysisDownloadUiMixin:
    ...
    def start_analysis(self):
        ...
        worker.progress.connect(self._analysis_progress_message)
    ...
    def _launch_download(self, request, settings: dict, *, from_queue: bool = False, mode: str = "selected"):
        ...
        worker.moveToThread(thread)
        thread.started.connect(worker.run)
        worker.status.connect(self._download_status)
        worker.log.connect(self._append_log)
        worker.stage.connect(self._download_stage)
        worker.progress.connect(self._download_progress_changed)
        worker.transfer.connect(self._download_transfer)
        worker.missing_media.connect(self._resolve_missing_media)
        worker.history_changed.connect(self._load_history)
        worker.request_changed.connect(self._download_request_changed)
