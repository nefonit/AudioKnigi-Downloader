...
# Critical accessibility packaging test.  This runs INSIDE the one-file EXE,
# not in the build virtualenv, and therefore catches missing Prism Python/native
# payloads that a pre-PyInstaller import check cannot detect.
$accessibilityReport = Join-Path $PSScriptRoot "dist\accessibility_frozen_selftest.txt"
Remove-Item -LiteralPath $accessibilityReport -Force -ErrorAction SilentlyContinue
$env:AUDIOKNIGI_ACCESSIBILITY_SELFTEST_REPORT = $accessibilityReport
Write-Host "Frozen accessibility smoke test: $exe --accessibility-import-selftest"
& $exe --accessibility-import-selftest
$accessibilityExit = $LASTEXITCODE
Remove-Item Env:AUDIOKNIGI_ACCESSIBILITY_SELFTEST_REPORT -ErrorAction SilentlyContinue
if (Test-Path -LiteralPath $accessibilityReport) {
    Write-Host "Accessibility frozen self-test report:"
    Get-Content -LiteralPath $accessibilityReport | ForEach-Object { Write-Host $_ }
}
if ($accessibilityExit -ne 0) {
    throw "The built EXE cannot load Prismatoid/Prism accessibility support (exit $accessibilityExit). See dist\accessibility_frozen_selftest.txt. The build is NOT valid for NVDA/JAWS."
}
if (-not (Test-Path -LiteralPath $accessibilityReport)) {
    throw "The built EXE did not produce the accessibility frozen self-test report. The build is NOT valid for NVDA/JAWS."
}
$accessibilityText = Get-Content -LiteralPath $accessibilityReport -Raw
if (-not $accessibilityText.StartsWith("OK")) {
    throw "The built EXE accessibility frozen self-test did not report OK. The build is NOT valid for NVDA/JAWS."
}

Write-Host "Build completed successfully with Python $RequiredPythonVersion x64 and frozen Prism accessibility support."
