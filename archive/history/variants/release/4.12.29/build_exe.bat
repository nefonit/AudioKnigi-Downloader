...
"%VENV_PYTHON%" -c "import importlib.metadata as m; from packaging.version import Version; v=Version(m.version('prismatoid')); assert v>=Version('0.18.2'); print('PyInstaller:', m.version('pyinstaller')); print('prismatoid:', v); from prism import BackendId, Context; import prism._prism_cffi as prism_cffi; import _cffi_backend; assert BackendId.NVDA and BackendId.JAWS; print('prism NVDA/JAWS API: OK'); print('prism._prism_cffi:', prism_cffi.__file__); print('_cffi_backend:', _cffi_backend.__file__)"
if errorlevel 1 goto :error
