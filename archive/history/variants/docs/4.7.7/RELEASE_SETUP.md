...
Windows release builds install `prismatoid>=0.17.3` and PyInstaller collects the `prism` package plus its distribution metadata. The runtime integration is lazy: if neither NVDA nor JAWS is active, no accessibility speech backend is opened and the application remains silent.
