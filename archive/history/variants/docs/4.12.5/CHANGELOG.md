## 4.12.5 — Keyboard & Dialog Accessibility

- Empty **Найти или открыть** submissions in simple mode now open an accessible modal dialog instead of placing a recovery card below the visible viewport.
- The validation dialog is centered over the app, modal, announced to the screen reader, supports Enter/KP Enter for **Перейти к полю**, and Esc/Close for dismissal.
- **Перейти к полю** closes the dialog and returns focus to the universal title/author/URL input; the scrollable home page automatically brings the focused field into view.
- Added a global native-ttk keyboard layer: Enter activates focused buttons/check/radio controls; F4 or Alt+Down opens comboboxes; Esc closes combobox popdowns.
- Esc now closes transient dialogs/dropdowns before cancelling an active download, avoiding accidental cancellation when the user only meant to dismiss UI.
- Added **Alt+1..5** for Book/Search/Queue/History/Settings and **Ctrl+Tab / Ctrl+Shift+Tab** for tab cycling. Ctrl+L still focuses the primary input.
- Search result tables support Enter to open a book and Shift+F10/Menu for context actions. Track, history and queue tables also expose keyboard context menus; history Enter opens the selected folder and track Enter starts playback.
- Updated keyboard-shortcut help in RU/UK/DE/EN.
- Added 11 focused 4.12.5 regression tests; the full pytest regression set contains 292 tests.
