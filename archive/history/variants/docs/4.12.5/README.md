...
### Keyboard and validation accessibility in 4.12.5

- Pressing **Найти или открыть** with an empty main field opens a centered modal dialog instead of a message below the current scroll position.
- **Enter** activates the primary dialog action and focused native buttons; **Esc** closes dialogs/dropdowns before it is used to cancel an active operation.
- **F4 / Alt+Down** opens drop-down lists, arrow keys navigate native tables/lists, and **Shift+F10 / Menu** opens context actions where available.
- **Ctrl+L** focuses the universal input; **Alt+1..5** open Book/Search/Queue/History/Settings; **Ctrl+Tab / Ctrl+Shift+Tab** cycle work tabs.
- Search results can be opened with Enter. Track, history and queue lists expose keyboard context actions while preserving native ttk focus and screen-reader semantics.

### Default 100% scale in 4.12.3

- A clean profile now starts at **100% UI scale** instead of 125%.
- The default is centralized in `DEFAULT_UI_SCALE`, so startup, settings restore and fallback behavior stay consistent.
- A scale already saved by the user is preserved. Existing profiles with `scale: 125` remain at 125 until the user changes the setting.
