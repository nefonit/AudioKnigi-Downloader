...
Начиная с 4.10.2 программа поддерживает языковые пакеты коротких звуковых подсказок. Для английского уже добавлены основные фразы скачивания/ошибки/поиска книги, а отсутствующие локализованные записи временно используют русский fallback. Звуки можно отключить, изменить громкость и проверить в **Настройки → Звуки программы**; они используют отдельный канал и не заменяют файл во встроенном мини-плеере.

### Поддерживаемые сайты
- `audioknigi.com.ua`
- `knigavuhe.org` — поиск через `/search/?q=...`, загрузка доступных книг по страницам `/book/...`.

Поиск во вкладке «Поиск» работает сразу по обоим источникам. Страницы, доступ к которым ограничен самим сайтом по просьбе правообладателя, программа не обходит.
