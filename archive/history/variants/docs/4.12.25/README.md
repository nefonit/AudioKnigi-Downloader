...
## 4.12.25 — надёжная проверка уже скачанных книг

Повреждённый `metadata.json` больше не отключает резервную проверку по истории, а новый `metadata.json` сохраняется атомарно. Текущий интерфейс остаётся только MP3.
