## 4.12.30 — Complete screen-reader interaction feedback

- NVDA/JAWS now announce state/value changes immediately after user actions, not only when focus arrives.
- Disabled action buttons are skipped by Tab; read-only text remains reachable and can be read line-by-line.
- Comboboxes use human labels instead of Tcl `PY_VAR...` names; notebooks announce current tab and position.
- Context-menu selection, track check/uncheck, bulk track selection, mode switching, sliders and buttons have explicit speech feedback.

## 4.12.29 — Prism CFFI frozen packaging fix

- Fixed the Windows frozen-build failure `ModuleNotFoundError: prism._prism_cffi`.
- PyInstaller now explicitly includes both `prism._prism_cffi` and CFFI's `_cffi_backend` as hidden imports and binary extension modules.
- Windows build preflight resolves the exact `.pyd` paths from the Python 3.14.7 build environment and passes them to PyInstaller with `--add-binary`.
- Frozen accessibility self-test now imports both CFFI modules and reports their extracted paths.
- Prismatoid minimum version raised to 0.18.2.
- Frozen self-test report falls back to `dist\accessibility_frozen_selftest.txt` beside the EXE, so diagnostics do not depend only on an environment variable.

# AudioKnigi Downloader

## 4.12.28 — NVDA/JAWS Frozen Prism Packaging Fix

- Windows PyInstaller builds explicitly collect Prism Python modules, native payload and metadata.
- The final EXE now runs a mandatory `--accessibility-import-selftest`; inaccessible builds are rejected.
- Runtime logs the real `prism` import traceback instead of masking it with an invalid `prismatoid` import fallback.


## 4.12.27 — NVDA/JAWS Tab Accessibility

- Fixed startup focus in Advanced mode: the app no longer tries to focus the hidden Simple-mode URL field.
- Added app-managed Tab/Shift+Tab traversal across visible enabled interactive controls, with hidden controls from the other UI mode excluded.
- Focused controls are announced to NVDA/JAWS with accessible name, role, state and current value through the Prismatoid bridge; dynamically-created controls are registered on first focus.
- Added **Ctrl+Shift+F12** screen-reader self-test and ACCESSIBILITY diagnostics in `app.log`.
- Updated Windows Prismatoid dependency to `>=0.18.1,<0.19`; Windows build scripts upgrade requirements and verify the NVDA/JAWS backend API.
- Windows EXE remains pinned to CPython 3.14.7 x64.

## 4.12.26 — полный аудит звуков событий и системный Windows fallback

Все 17 встроенных голосовых событий проверены; 16 имеют реальные runtime-триггеры, а `update_available` остаётся подготовленным до появления настоящей проверки обновлений. На Windows системные звуки теперь используются для служебных событий и как резерв, если голосовой MP3/pygame недоступен.


### Звуки событий

Начиная с 4.10.2 программа поддерживает языковые пакеты коротких звуковых подсказок. В 4.10.4 английский пакет полностью покрывает все 17 зарегистрированных голосовых событий. В 4.12.26 добавлен Windows system-sound fallback: ваши MP3 остаются приоритетными, а системный сигнал используется для служебных событий и если MP3/pygame недоступен. Общий переключатель отключает оба типа подсказок. Ползунок громкости регулирует только голосовые MP3; системные звуки используют громкость Windows. В **Настройки → Звуки программы** есть отдельные проверки «Голосовой» и «Системный».
...
- Settings sliders for bandwidth and event-sound volume now jump directly to the clicked point on the track instead of moving one step and auto-repeating while the mouse is held. Thumb dragging and keyboard control remain native.
- Queue completions remember the actual MP3 paths produced for each item.
- Every **Start Queue** locally revalidates items marked Done: existing non-empty MP3 files keep the item Done; deleted/missing/zero-byte files return the item to Pending so the queue downloads them again.
- The completed-file check is local-only and does not add a network request before queue start.
- Added `QUEUE FLOW | event=completed_files_missing` diagnostics and focused regression coverage.
...
## 4.12.28 — NVDA/JAWS frozen build hardening

Windows builds now run a mandatory Prism accessibility self-test from inside the finished one-file EXE. If the `prism` package, its native payload, or NVDA/JAWS API identifiers are missing after PyInstaller packaging, the build fails instead of producing an inaccessible EXE. See `ACCESSIBILITY_FROZEN_FIX_4_12_28.md`.