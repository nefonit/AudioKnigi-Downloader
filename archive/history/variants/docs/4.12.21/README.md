...
## 4.12.21 — MP3-only and complete Help Center

- Removed the M4B output path from the current application. AudioKnigi Downloader now has one final audio format: **MP3**. Legacy saved output-format values are ignored/migrated safely and are no longer exposed in the UI.
...
- Windows EXE build scripts remain strictly pinned to **CPython 3.14.7 x64**. Source validation in this environment may use another Python/runtime and is reported separately.
